...
resource "google_compute_network_peering" "frontend_to_backend" {
  ...
  network      = module.vpc1_frontend.vpc_self_link
  peer_network = module.vpc2_backend.vpc_self_link
}

resource "google_compute_network_peering" "backend_to_frontend" {
  name         = "backend-to-frontend"
  network      = module.vpc2_backend.vpc_self_link
  peer_network = module.vpc1_frontend.vpc_self_link
}

# Firewall rules for VPC peering communication
resource "google_compute_firewall" "backend_allow_frontend" {
  name    = "vpc2-backend-allow-frontend"
  network = module.vpc2_backend.vpc_name
  ...
  source_ranges = ["10.0.2.0/24", "10.0.1.0/24"]
}

# VPC Peering: Frontend <-> Bastion
resource "google_compute_network_peering" "frontend_to_bastion" {
  name         = "frontend-to-bastion"
  network      = module.vpc1_frontend.vpc_self_link
  peer_network = module.vpc_bastion.network_self_link
}

resource "google_compute_network_peering" "bastion_to_frontend" {
  name         = "bastion-to-frontend"
  network      = module.vpc_bastion.network_self_link
  peer_network = module.vpc1_frontend.vpc_self_link
}

# VPC Peering: Backend <-> Bastion
